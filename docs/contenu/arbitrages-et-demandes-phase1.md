...
**Contexte.** Quatre piliers portent les durées héritées de S1 (5/10/20 min) faute de calibrage propre. *(Signal salve C3, 16 sept : l'écran acte libre n'affiche aucune durée — le défaut est invisible pour l'utilisateur, l'arbitrage perd son urgence.)* Cohérent avec leur type « acte libre » (pas de timer), mais c'est un défaut technique, pas une décision produit. S2 (30/45/60), S4 (5/20/45) et S6 (15/30/60) ont leurs durées propres.
...
1. **S2 Activité physique — validation résiduelle.** *(Réduit le 16 sept 2026 : la matière Jacky enrichie — section « ÉVALUATION DE DÉPART » — a été intégrée au code : 12 questions et labels des 5 niveaux = Jacky, actés.)* Restent à valider avec lui : (a) les **5 messages de diagnostic** (drafts Claude sous ses labels), (b) le **sens de Q7** « Je transpire facilement quand je fais un effort » (codée directe en attendant — bon ou mauvais signe ? Signal salve C2 : Stéphane lui-même l'a cotée comme négative en se testant), (c) le **calibrage des durées** codées 30/45/60 min vs ses paliers par type de séance (20-90 min), (d) le **seuil du niveau 5** (>50/60 : un « 4 partout » donne niveau 4 « fonctionnelle » — exigeant, cohérent avec le principe 8, à confirmer comme voulu). Lui soumettre le brief `brief-pilier-s2-activite-physique-v1.md` annoté par la salve (bloc C2). *(Tâche kanban R4-13.)*
