...
**Vérification F-07/K1 (22 sept 2026).** Migration `20260922_f07_dev_tools_per_account.sql` appliquée par Stéphane. `information_schema.column_privileges` sur `profiles` pour `authenticated`/UPDATE : exactement 8 colonnes (account_created_at, current_pillar_id, narrative_flags, onboarding_data, onboarding_done, pending_tier_reach, pillar_started_at, profile_dynamic_id) — `dev_tools_enabled` absent, verrou en place. Code au commit `09ecce0`, décision D43 actée (CLAUDE.md V1.7).
...
**Étape 7 (22 sept 2026) — exécution terminée.**

- **F-10** (commit `eca9424`, fonctions redéployées et vérifiées en prod le jour même) : CORS de `stripe-portal` restreint à une liste blanche (PWA prod, previews Vercel, localhost) — vérifié par requêtes OPTIONS live (origine autorisée → header ACAO, origine inconnue → bloqué. Le rapprochement email du webhook pagine désormais toutes les pages `listUsers` (le plafond 50 du rapport avait déjà été durci à 1000 par R2-16). `@expo/ngrok` déplacé en devDependencies. `npx expo install --fix` + `npm audit fix` : advisory query-string/decode-uri-component du rapport soldée ; restent image-size/postcss/uuid (transitifs de l'outillage de build, fix breaking upstream, risque faible). Deux ajustements forcés : eslint-config-expo maintenu en ^57 (l'alignement Expo le downgradait et retirait les règles react-hooks v6) et jest-expo épinglé 55.0.17 exact + peer `test-renderer` explicite (la montée 55.0.22 cassait la résolution du renderer de la suite).
- **F-12** (commit `c22435b`) : release Sentry dérivée de la version package.json (`rawadventure@<version>`) au lieu de la valeur en dur ; plugin `@sentry/react-native/expo` configuré (organization `adventure-limitedraw`, project `rawadventure`) pour l'upload des source maps — actif dès que `SENTRY_AUTH_TOKEN` est posé dans l'environnement de build.
- **F-13** (commit `d45feb6`) : inventaire des 20 placeholders `[copy à valider]` livré dans `docs/contenu/copy-a-valider-inventaire.md` (pour validation Mimi & Jacky) ; les notices streak/joker sorties de la logique vers `src/data/global-copy.ts` (slots `copy.global.*`, D23), verrouillées par 5 tests écrits rouge d'abord.
- **F-17** : suite mesurée à ~13 s à chaud / ~27 s cache froid (573 tests, 37 suites) — loin des 30 s constatés chez Lou, pas d'optimisation nécessaire ; chiffre « < 10 s » du CLAUDE.md § 7 corrigé.
- **F-16** était déjà couvert au fil des fixes (tests de spécification citant les décisions) ; **F-11** reste raccroché au chantier comptes stores (R6).

**Bilan final : les 17 findings du rapport sont soldés, caducs ou raccrochés à un chantier tracé.** F-01→F-10, F-12, F-13, F-16, F-17 exécutés ; F-14 caduc (D42) ; F-15 reporté (décision produit notifications PWA) ; F-11 suivi dans R6. Écart méthodologique assumé à mentionner à Lou : F-05.5 compose des hooks de domaine sous un provider unique au lieu de trois Contexts séparés (zéro churn sur les 19 consommateurs, chemin d'upgrade mécanique préservé).
