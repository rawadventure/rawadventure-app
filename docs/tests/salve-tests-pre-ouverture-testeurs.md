...
9. **Blocage ponctuel sur le récap d'éval initiale** (relevé Stéphane, 7 sept, +demo4 pendant S1) : coincé sur « Ton diagnostic respiration », « Continuer » et navigation inertes ; R2 a tout débloqué, non reproduit depuis. Investigation du 7 sept : logique et navigation réelles vérifiées saines (test PillarRecapNavigation ajouté, vraie stack HomeV1→PillarRecap + press Continuer → hub Phase 1 vert) ; modale niveau hors de cause (démontage propre). Une seule occurrence — À SURVEILLER : si ça se reproduit, noter l'enchaînement exact (modale niveau ouverte ou pas, +1j actif ou pas) avant le R2.
10. **Ton de la charnière après une journée « au rabais »** (relevé Stéphane, 8 juillet) : une journée validée à 2/7 avec joker déclenche quand même le texte enthousiaste de la charnière (« Le corps commence à répondre ») — mécaniquement voulu (D38), variante de copy à envisager avec Mimi & Jacky.
