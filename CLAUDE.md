...
*Daté du 22 septembre 2026 — Version 1.7 (ajout de D43 : panneau DEV par compte, révise la posture reset § 2.11). À mettre à jour à chaque décision structurelle nouvelle.*
...
**Révision D43 (22 septembre 2026, F-07/K1 audit Lou Grenier).** Le panneau DEV (reset complet, snapshots timeline, horloge virtuelle, mock abonnement) est activé par `__DEV__` **ou par compte** via la colonne `profiles.dev_tools_enabled` (défaut `false`), lue au chargement du profil (`isDevToolsEnabled()` dans `src/lib/devToolsEnabled.ts`). L'ancien flag env Vercel `EXPO_PUBLIC_ENABLE_DEV_PANEL` est retiré du code — il exposait le panneau à tout le déploiement testeurs. La colonne est verrouillée côté base (privilèges de colonne Postgres, migration `20260922`) : seul le SQL editor peut flagger un compte. En production, le reset n'est donc exposé qu'aux comptes explicitement flaggés par Stéphane.
...
**D42** — **Trois durées d'abonnement** (8 septembre 2026, actée par Stéphane pendant le bloc S de la salve, avec Mimi). L'offre unique se décline désormais en **trois durées : 49 €/mois, 239 €/6 mois, 399 €/an** — révise D11 (qui limitait à mensuel + annuel). Aucun impact code V1 : l'app n'affiche aucun prix (pattern Reader App), seules la page d'abonnement web et le portail client Stripe les exposent. Le portail client autorise le changement d'offre entre les trois durées, quantité verrouillée, « pas de frais ni de crédits » au changement (application au prochain cycle).
...
**D43** — **Panneau DEV par compte** (22 septembre 2026, F-07/K1 de l'audit Lou Grenier, principe validé par Stéphane le 18 septembre). Les outils DEV sont activés par `__DEV__` ou par la colonne `profiles.dev_tools_enabled` (verrouillée côté client, flaggable uniquement en SQL editor). Le flag env Vercel testeurs est supprimé. Révise la posture reset du § 2.11 (A2) : en production, seuls les comptes flaggés voient le panneau.
